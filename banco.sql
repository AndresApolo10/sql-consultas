...
select accounts.account_no, accounts.client_id, account_types.name as type_account from accounts right join account_types on accounts.id = account_types.id;

select transactions.id, transactions.amount, transactions_types.name as type_transaction from transactions inner join transactions_types on transactions.id = transactions_types.id;

select transactions.id, transactions.amount, transactions_types.name as type_transaction from transactions_types left join transactions on transactions_types.id = transactions.id;
...
create view v_accounts_types as select accounts.account_no, accounts.client_id, account_types.name as type_account from accounts right join account_types on accounts.id = account_types.id;

create view v_transactions_detail as select transactions.id, transactions.amount, transactions_types.name as type_transaction from transactions inner join transactions_types on transactions.id = transactions_types.id;

create view v_transations_types_detail as select transactions.id, transactions.amount, transactions_types.name as type_transaction from transactions_types left join transactions on transactions_types.id = transactions.id;
